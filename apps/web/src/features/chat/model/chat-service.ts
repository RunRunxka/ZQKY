/**
 * 统一聊天事件服务：真实 SSE 与本地模拟驱动同一套状态与消息组件。
 *
  ...
  return {
    ...
    async run(request, emit) {
      ...
      if (capability && capability.value)
        composerNotes.push(`本轮能力：${capability.label}（配置：${capabilitySummary || '未携带'}）`);
      ...
      if (capability?.value === 'deep_research') {
        ...
        const outlineAnswers = await waitForReply(pendingReplies, outlineInteractionId, request.signal);
      ...
      if (capability?.value === 'visualize') {
        ...
        if (resolved === 'manim_video' || resolved === 'manim_image') {
          ...
          await runStage('code_generation', '生成 Manim 代码', null, async () => {
            emit({ ...turn, type: 'process', delta: '[模拟] 已生成 Scene 代码（本地演示，不执行 Manim）。' });
          });
          await runStage(
            'code_retry',
            '渲染重试',
          ...
          await runStage('summary', '总结', '[模拟] 动画要点：概念引入、变换过程、结论强调（演示总结）。');
          ...
          await runStage('render_output', '渲染输出', null, async () => {
            if (isImage) {
              ...
              emit({ ...turn, type: 'process', delta: '[模拟] 已生成分镜示意图（演示内容，非真实渲染帧）。' });
            } else {
              emit({
                ...turn,
                type: 'process',
                delta: '[模拟] 视频渲染服务未接入：不生成真实视频文件，仅提供代码、分镜与总结（如实说明，不假装渲染）。',
              });
            }
          });
          emit({
        ...
        await runStage('generating', '生成渲染内容', null, async () => {
          if (resolved === 'chartjs') {
            ...
            emit({ ...turn, type: 'process', delta: '[模拟] 已生成 Chart.js 配置（预览由本地 Chart.js 渲染，数据为演示值）。' });
            ...
            data = { source: content };
            emit({ ...turn, type: 'process', delta: '[模拟] 已生成 Mermaid 源码（预览由本地 Mermaid 渲染）。' });
            ...
            emit({ ...turn, type: 'process', delta: '[模拟] 已生成静态 HTML（sandbox 隔离预览，禁脚本）。' });
          } else {
            kind = 'svg';
            content =
