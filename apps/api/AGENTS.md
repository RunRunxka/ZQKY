...
- 凭证与密钥只存在于服务端环境；进程内 `core/secrets.py` 是唯一凭证存放点，日志、错误、health/capabilities、配置文件均不得包含任何凭证或配置值。`.env.example` 只放无敏感信息示例。
...
- 流式约定（D04）：SSE 事件只用 message.start/text.delta/usage/message.end/error；流开始前的失败返回 HTTP 错误，流开始后失败发 `error` 事件；上游结束原因规范化为 stop/length/unknown；生成器 finally 必须关闭上游连接使取消传播；对话内容不落盘、不进日志。
- 变更后运行 `npm run test:api`；涉及启动方式或接口时同步根 README、docs/API.md、DECISIONS、TASKS、HANDOFF。
