...
| 当前学习问答视觉基准与固定功能参考 | 2026-09-10 用户要求全站外观以当前学习问答为准；业务信息结构/交互仍对照固定 DeepTutor。scout 读取现行 PROJECT_GUIDE，不照搬本提案历史范围。 |
...
以上是提案编写时的历史发现；2026-09-10 文档交接批已整理这些过期条款，当前断点只看 STATUS。取证者仍须报告实际冲突来源，总控按最新明确授权和作用范围判断，不按日期、目录深度或一个状态标签机械决定。
