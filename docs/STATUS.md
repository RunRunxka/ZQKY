...
更新：2026-09-22（main 审查与下一阶段/RAG 接入规划；未实施产品修复或 RAG 接入）。本文件是唯一进度、问题、任务和后续计划入口。长期目标与稳定决定见 [PROJECT_GUIDE](PROJECT_GUIDE.md)，逐项范围见三矩阵，历史首败与批次全文见 [整理前完整快照](archive/DELIVERY_HISTORY.md#snapshot-status-20260915)。

## 1. 目标与当前结论

**以智启课源品牌完成固定 DeepTutor 的产品前端、AI 交互与原有动画；全站以当前学习问答 `/chat` 为视觉基准，保留蓝色主题及原业务功能。整体尚未完成。**

**当前工作：main 分支代码审查与后续计划，产品实施未启动。** 最近已实施批是 H1-BOOKS-PIPELINE v2（2026-09-20 本地实现与限定范围验收，见 §5.0 与 [qa 任务卡](qa/H1-BOOKS-PIPELINE/TASK-CARD.md)）。2026-09-22 审查发现 M22-01～06 仍未修复，详见 §3.1；历史 pass 保留，不能覆盖新发现。**书籍仍为本地模拟执行器，不接真实 LLM/解析，不代表书籍模块或全站完成。**

R-05 内容视觉统一是 H1 之前已交付的批次。第六批 B-R05-EXTEND v5（2026-09-19）覆盖阅读工作区和 `/space` 四子页，并修复 R-09 滚动竞争。历史独立验收 A1 pass 0 fail 保留，R-05 只在 §5.3 登记范围内关闭，不扩大为全站逐状态完成。建议先做 H1-BOOKS-HARDEN，再推进课程和 RAG 接入，见 [当前批次与下一动作](#current-task)。

- 本次审查代码基准：`main@7a394ef`，开始工作区干净；最近主要业务实现为 `f31d39f`，之后产品差异为公共导航字体。`main` 是个人分支，默认集成分支是 `feat/glass-theme`，本报告不计入后者的主题工作。现场分支/HEAD 必须重查，不自动切换或合并。
...
| ID | 未修复项 | 证据层级 |
| --- | --- | --- |
| M22-01 | 读取失败或删除后执行器退出但注册表/心跳/租约未释放，恢复被旧句柄阻塞 | jsdom 隔离复现 2 例 |
| M22-02 | 页/块修复不返回 Promise，且旧修复任务等待后借用新 runId 写入 | jsdom 隔离复现 1 例；错误提示/忙态源码核对 |
| M22-03 | 租约非原子获取、无失权停止，多标签同时启动存在竞争 | 源码风险；真实同时竞争待复现 |
| M22-04 | 首次目录读取失败被书籍详情加载分支遮住，缺错误/重试 | 源码确认；浏览器待补 |
| M22-05 | 最终完成落库异常被吞掉，内存仍 finished | 源码确认；最终写入故障待补 |
| M22-06 | 编辑笔记时左右方向键触发全局翻页 | 源码确认；键盘浏览器待补 |
...
### 5.A 建议下一实施批：H1-BOOKS-HARDEN v1（尚未启动）

目标：修复 §3.1 M22-01～06，保持书籍七态、14 类 block、笔记/练习/书签/导出、显式模拟标注及自动接管既定语义。先复现再修；不得只修改断言掩盖问题。

- 总控独占契约、文档、Git 与集成构建；引擎负责人独占 `book-generation.ts`、必要 `books-store.ts` 及相关单测；UI 负责人独占 BooksRoute/PageReader 与对应测试；先明确 Promise/操作身份/取消合同，再并行，独立验收者只读稳定候选。
- 核心出口：读拒后可恢复，删除无心跳/监听残留，最终写失败不假完成；修复任务固定 runId/操作版本并拒绝迟到写；真实双标签同时启动/失权/不同书并发不串写；输入框方向键不翻页、普通阅读快捷键有效；旧数据与用户编辑不丢。
- 测试：3 个本次缺陷探针改造成正式回归时反转为期望行为；补真实双标签与失败恢复 E2E。typecheck/lint/unit/build/相关浏览器测试，候选冻结后全量 E2E；后端零改动可引用本轮 API 基线，后端改动须重跑。
- 资源：先核占用；建议引擎隔离内存数据，UI 自测 5175，集成 E2E 5174，构建目录总控单写。不得操作正式 `.env`/`.local-data` 或用户浏览器草稿。
- 不包含：课程新功能、真实 RAG 接线、主题分支合并、LLM 预算调整。本提示为下一批建议，接收用户执行指令后落地，不以本次规划冒充实施授权。
...
| 顺序/轨道 | 交付中心 | 出口 |
| --- | --- | --- |
| **先修 H1-BOOKS-HARDEN v1** | 七态流水线已实现，先补 M22-01～06，避免重复重写已交付批次 | 故障恢复、并发归属、数据保护与独立复验 |
