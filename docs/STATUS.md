...
4. **下一动作（顺序）**：① 跨库存储保护加固（books/courses/knowledge/space/notebook 的 R27 同类缺陷，P1）；② 知识导入/解析/索引模拟阶段闭环；③ 书籍生成流水线状态（compiling/paused/error + 流式过程）；④ 课程学习会话联动；⑤ S5-F→G→H→I→S6→S7→S8。有界差距核查结论见 [GAP_AUDIT_2026-09-09](replica/GAP_AUDIT_2026-09-09.md)。
