...
4. **下一动作（顺序）**：① 跨库存储保护加固（books/courses/knowledge/space/notebook 的 R27 同类缺陷，P1）；② 知识导入/解析/索引模拟阶段闭环；③ 书籍生成流水线状态（compiling/paused/error + 流式过程）；④ 课程学习会话联动；随后 S5-E→F→G→H→I→S6→S7→S8。有界差距核查结论见 [GAP_AUDIT_2026-09-09](replica/GAP_AUDIT_2026-09-09.md)。
5. 不以“没有真实后端”删除前端验收项；媒体播放器原视图与真实解析保持显式模拟边界。

没有授权调用的真实供应商或不可用外部服务不阻塞其余前端工作；最终分别写“前端模拟可验收”与“真实服务未验证”。

## 最近一轮实际检查（R26–R31 + R32 + 书籍差距收口，2026-09-09）

| 检查 | 结果/边界 |
| --- | --- |
| typecheck / lint | 通过 / 0 警告（三批各复跑） |
| 正式单测 | **227/227 通过**（第一批后 214、第二批后 225、第三批 +2：书籍作答持久化与 block 品类覆盖；Node 26 复跑需 `NODE_OPTIONS=--no-experimental-webstorage`） |
| build | 通过（每批一次） |
| 独立探针复跑 | 存储 3/3、浏览器 4/4 通过（第一批后，`_work/reading-fix-20260909/`） |
| 阅读相关 e2e | 15/15 通过（第二批，含 R28–R31 迁移回归与 R32 ×3） |
| 正式全量 e2e | 第一批后 **96/96**；第二批后 **99/99**；第三批后 **99/99**（`_work/reading-fix-20260909/`、`_work/reading-r32-20260909/`、`_work/books-r32-20260909/`） |
| 模型目录 ECONNREFUSED | 环境边界：本轮未启动后端（8000），与审查轮一致，不视为真实供应商验证 |
| 真实供应商/MCP/Skills/解析/STT | 未调用，未验证 |
| test:api | 未执行，后端未改 |
| Git 事件 | 第一批提交后发生本地分支 ref 文件丢失（H: 盘同步盘异常），经 reflog + update-ref 恢复至 6fbd542 并 fsck 通过；后续每批提交后校验 ref 在位 |
