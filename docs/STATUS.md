...
4. **下一动作：S3/S4/S5-A～C 有界差距核查并落入矩阵**（书籍 block 分发/暂停恢复/练习答案、课程会话联动、知识导入索引模拟、聊天产物跨页联动），按独立小批补齐；随后 S5-E→F→G→H→I→S6→S7→S8。
5. 不以“没有真实后端”删除前端验收项；媒体播放器原视图与真实解析保持显式模拟边界。

没有授权调用的真实供应商或不可用外部服务不阻塞其余前端工作；最终分别写“前端模拟可验收”与“真实服务未验证”。

## 最近一轮实际检查（R26–R31 修复 + R32 补齐，2026-09-09）

| 检查 | 结果/边界 |
| --- | --- |
| typecheck / lint | 通过 / 0 警告 |
| 正式单测 | **225/225 通过**（第一批后 214；第二批新增 companion-service/reading-ingest/会话草稿与状态归一化 11 条；Node 26 复跑需 `NODE_OPTIONS=--no-experimental-webstorage`） |
| build | 通过（两批各一次） |
| 独立探针复跑 | 存储 3/3、浏览器 4/4 通过（第一批后，`_work/reading-fix-20260909/`） |
| 阅读相关 e2e | 15/15 通过（第二批，含 R28–R31 迁移回归与 R32 ×3） |
| 正式全量 e2e | 第一批后 **96/96**；第二批后 **99/99**（`_work/reading-fix-20260909/`、`_work/reading-r32-20260909/`；上轮 chat-motion teardown 超时未再复现） |
