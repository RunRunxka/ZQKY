...
| `npx playwright test`（全量 e2e，202 例） | **201 通过 / 1 失败**：唯一失败为 `course-sessions.spec.ts:427`「流式中切换会话」（第 461 行点击侧栏「新对话」后 URL 未切换），**不可稳定复现**（隔离 `--repeat-each=3` 3/3 通过、整文件 `--repeat-each=2` 22/22 通过）→ 已登记 **R-17**（间歇；机制假设未证实；本批改动与 store/会话创建零交集） |
