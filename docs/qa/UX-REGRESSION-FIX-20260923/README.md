...
**教案工作台顶栏**、**教材资料库到书籍/课程的返回路径**。独立验收见 [A1（r2，pass 可交付）](A1-REPORT-01.md)；任务卡与文件归属见 [TASK-CARD.md](TASK-CARD.md)。
...
| 帧 > 50 ms / > 100 ms | 215 / 112 | 0 / 0 | **0～1 / 0**（本机多次测量：优化后两轮 0/0，独立验收两轮 1/0，单帧 ~95–100 ms） |
| 长任务 | 210 条 / 23 078 ms | 0 条 | **0～1 条**（独立验收两轮各 1 条 × 80 ms；同机状态相关，非本修复引入） |
...
口径说明：**帧间隔与长任务是"同机状态相关"的量化指标**——优化后本机两轮为 0 帧 > 50 ms / 0 长任务，
独立验收在同一候选上两轮各出现 1 帧（max ~95–100 ms）与 1 条长任务（80 ms）；相对优化前基线
（215 帧 / 210 条）仍是两个数量级的改善，故结论不变，但不再写成"恰为 0"。本修复为「流式期间显示公式」付的代价是**主线程脚本 +1.7 s**（每次提交解析的尾段上界 4000 字符，
...
| `npm run test:unit` | **54 文件 / 459 例通过**（上一批 52/441；本批新增 18 例）。**必须带 `NODE_OPTIONS=--no-experimental-webstorage`**：Node 26 的实验性 Web Storage 会让裸跑的 `localStorage` 为 undefined，裸跑会看到 216 例失败（独立验收已复现并记录） |
| `npm run build` | 通过（`BUILD_ID` 见 FROZEN 记录） |
| `npm run test:chat`（流式集成，fixture 真实代理） | 9 例中 **8 通过 / 1 失败**（失败为既有陈旧用例，见 §5） |
| `npx playwright test`（全量 e2e，202 例） | **最终候选（含手机顶栏间隔修复）：202 通过 / 0 失败**（6.6 m，**单次运行**）。此前同一批次在未含手机修复的构建上为 **201 通过 / 1 失败**——唯一失败为 `course-sessions.spec.ts:427`「流式中切换会话」（第 461 行点击侧栏「新对话」后 URL 未切换），**不可稳定复现**（隔离 `--repeat-each=3` 3/3、整文件 `--repeat-each=2` 22/22 通过）→ 已登记 **R-17**（间歇；机制假设未证实；本批改动与 store/会话创建零交集）。**独立验收在同一候选上得 201 通过 / 1 失败，唯一失败是跨批 R-14**（`books-commit-safety:238`，失败快照为既有的「生成已中断 + 继续生成」如实状态、笔记与进度完好）→ 因此「202/0」只代表本机单次运行，不是"该候选恒绿" |
