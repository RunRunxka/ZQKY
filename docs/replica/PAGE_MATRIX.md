...
更新：2026-09-24（CHAT-CONTENT-MATH-AND-FOLLOW v1 更新 P-chat 助手正文流式安全数学与 reasoning 跟随证据，候选待独立验收；不升级功能/视觉标签）。此前 UX-REGRESSION-FIX v1 对 P-chat 推理子项及 P-lesson-plans/P-books/P-courses 的记录保持历史原样，UX-PERF-CLOSEOUT v1 更新条目亦不升级验收状态。全站视觉以当前学习问答为准；模块信息结构/功能/动画对照固定 DeepTutor。下表“状态”是**功能状态**；页面内容是否按学习问答完成视觉验收在文末单列，二者不得互相替代。根级 NavigationPreference 的公共壳证据不代替各页面内容证据。
...
2026-09-22 main 审查：`P-books-[bookId]` / `P-books-pages-[pageId]` 保持部分实现，新增读取失败加载不退出、修复任务归属与忙态、方向键干扰编辑等未修复项；参见 [M22-01～06](../qa/main-review-20260922/README.md)。历史视觉部分验收保留，本轮未重新进行浏览器/视觉验收。**2026-09-22 H1-COURSE-SESSIONS v1** 补齐课程学习会话闭环（`P-courses-[courseId]` 新增会话区、`P-chat-[sessionId]` 显示课程归属并返回课程），并新增 `BOOKS-CS-FOLLOWUP v1` 书籍前置补丁（缺 Web Locks 不降级、控制操作按提交结果分支）。**同日 H1-BOOKS-COMMIT-SAFETY v1** 进一步收口书籍集合的**保存一致性**（互斥锁内事务读改写 + 提交结果契约；创建/编辑/删除/笔记/作答/进度/生成/修复全部同一协议，失败保留输入可重试）——见 [CS 批次证据](../qa/H1-BOOKS-COMMIT-SAFETY/README.md)；该批**不改变任何页面的视觉/动画**，也不升级功能标签。**同日 H1-BOOKS-HARDEN v1 已修复这六项**（统一收尾、修复身份冻结与互斥、租约归属与共享集合写冲突收敛、读取失败可见可重试、最终落库失败不假报完成、编辑器方向键不翻页），证据见 [批次证据](../qa/H1-BOOKS-HARDEN/README.md)；本次修复**不升级**任何页面的视觉/动画标签（本批未做新的视觉验收），也未改变“部分实现”的功能边界（BookChatPanel、真实 LLM/解析仍缺；课程学习会话已由 H1-COURSE-SESSIONS v1 补齐，见本轮前句）。
...
| P-chat | `/chat` | `/chat` | 部分实现 | 当前全站视觉基准：220/56px 导航（导航内可滚动「学习记录」区域）、912px 内容上限、输入区图标/字体/思考球与模型弹层。普通真实聊天及本地历史已有；运行时模拟服务与切换入口已删除，R-03生产mock残留已清理，复杂能力真实通道未接。历史主页和 H0 侧栏回归不代表全产品或真实供应商通过。**CHAT-CONTEXT-BUDGET v1（2026-09-23）**：请求发送前统一预算与课程字段限幅，账目随消息持久化（字符估算）。**UX-REGRESSION-FIX v1（2026-09-23，历史子项证据）**：推理流公式即时呈现，历史三协议 reasoning fixture 通过；不覆盖助手正文 `message.content` 流式路径，也不等于用户真实供应商通过。**CHAT-CONTENT-MATH-AND-FOLLOW v1（2026-09-24，独立验收 pass，附 not_run 条件）**：streaming body 接入有界安全 Markdown 分段，闭合公式流中可见、未闭合尾段保持原文、终态/刷新后渲染，落库原文逐字节不变；拆分 reasoning `working` / `autoExpand`，正文出现折叠仍保留，流中用户重开后 follow 继续，鼠标上滚保留位置、回底恢复（独立验收以真实滚轮复现“未被抢回”与“回底恢复”）。性能受控 20k/50k 样本 p95 5.7ms、0 帧 >50ms、0 长任务，验收另证非“每 delta 重解析整段正文”。以上浏览器链路均使用合成 SSE；真实供应商及人工视觉 not_run。**UX-PERF-CLOSEOUT v1（2026-09-23）**：学习记录并入全站导航、模式菜单与 RAG 占位、长流性能及字体 token。所有批次**不升级 P-chat 功能/视觉标签**。**RAG-DELIVERY-v2（2026-09-26）**：/chat 的两个教材入口已接独立本地通道并三视口实测；默认只发布教材原文摘录与定位，人工质量 not_run。 |
| P-chat-[sessionId] | `/chat/[sessionId]` | `/chat/[sessionId]` | 实现待验收 | 与当前主页共用外观，真实会话深链/恢复/无效id处理已有；旧mode参数不恢复模拟。完整消息交互、历史数据与跨页来源待核验；侧栏父菜单高亮由 WorkspaceShell 统一。**H1-COURSE-SESSIONS v1（2026-09-22）**：会话带课程归属时显示「所属课程：…」与「返回课程」，课程删除/目录读取失败如实标注且不回落其他课程；发送时把课程快照冻结为一条 `system` 消息进入真实请求链路（重试沿用原快照、新轮用新快照）；切换会话不残留课程上下文。 |
...
| P-lesson-plans | —（目标自有） | `/lesson-plans` | 已验收 | 目标项目既有教案工作台，保留独立地址与导航入口；首页现为 /chat；e2e `lesson-plan.spec.ts` 回归。**B-R05-EXTEND v4（2026-09-19）视觉推广**：表单分区/type-chip/模板卡视觉、预览工具栏 hover/disabled、toast 与 storage-alert 视觉（文案与 role 未动）、导航分段微调、导出菜单进场 180ms（对齐 /chat 基准）。**导出与草稿链路零改动**（`print.css`、`@page lesson-plan`、Word 映射、防抖/flush、`pagination.ts`、`--serif` 回退链）；A1 复核 `verifyDocx` 全项通过、损坏草稿不覆盖。 **UX-REGRESSION-FIX v1（2026-09-23）**：顶部标题改为进入公共 `app-header`、与「导出教案」**同一行**（删除独立标题栏 `.lesson-page-head`、其网格行与失效 CSS），四视口实测标题与导出同行、无重叠、无横向溢出、内容区紧接顶栏（编辑区 top＝顶栏底）；单一折叠按钮、面板顺序、过渡动画、草稿/撤销/预览/打印/导出均未改。 |
...
| P-books | `/books` | `/books` | 部分实现 | 统计/目录/演示无损载入/新建模拟提案/状态徽标/进度/删除已有。**H1-BOOKS-PIPELINE v2（2026-09-20）**：生成不再是"确认即同步 ready"——卡片对生成中/已暂停/生成失败/可阅读四态可见（生成中带脉冲圆点 + `n/m 章` 进度）；历史用例 books-courses.spec.ts 覆盖列表切片。**B-R05-EXTEND v2（2026-09-18）视觉推广**：卡片悬浮语言（150ms 抬升/阴影/箭头位移）、搜索 ≥640px 常驻 + 匹配计数行、删除改两击确认、章/页/时间图标化、异步按钮 busy；统计 chips 结构按队长裁定保留；**UX-REGRESSION-FIX v1（2026-09-23）**：列表页头部新增同级样式、可访问的「返回教材资料库」链接（固定 `/knowledge-bases`，不依赖 `history.back`，深链直开亦可返回）；详情页既有「返回书籍列表」保留；e2e 真实点击验证「教材资料库 → 书籍列表 → 书籍详情 → 书籍列表 → 教材资料库」全路径与唯一当前项。 |
| P-books-[bookId] | `/books/[bookId]` | `/books/[bookId]` | 部分实现 | 提案确认→大纲确认→阅读器状态分流、无效id、续读定位、重建确认、Markdown导出已有。**H1-BOOKS-PIPELINE v2（2026-09-20）**：确认大纲后进入异步流水线（活动条 46px + 阶段文案/章数/计时 + 展开详情；暂停横幅；paused 不自动续跑；已中断给「继续生成」；整轮失败给原因与「重试生成」；归档书生成入口禁用）；BookChatPanel 待补；不再把14类分发与作答保存列作从零开发。**B-R05-EXTEND v2（2026-09-18）**：确认提案/确认大纲 busy 态、模拟编译加载提示、侧栏长章节名截断、书签图标化、已读/未读视觉区分；窄视口 `≤900px` 解除侧栏吸顶（B-R05-EXT2-A1 实测 pass）；侧栏折叠/HealthBanner/Chat 按裁定属 H1 | **H1-BOOKS-HARDEN v1（2026-09-22）**：该页首次读取目录失败不再被加载分支掩盖（错误面板 + 「重试读取」+ 成功清旧错误），执行器退出统一收尾（删除先停任务、读失败/失权如实提示并可恢复），最终完成写入失败不假报完成（保留「重试生成」）。 **H1-BOOKS-COMMIT-SAFETY v1（2026-09-22）**：创建/确认/重建/删除等写操作以仓储提交结果为准（`committed` 后才关表单、跳转、提示成功）；持锁冲突时如实提示并保留用户输入，可重试。
| P-books-pages-[pageId] | `/books/[bookId]/pages/[pageId]` | 同 | 部分实现 | BookBlockType/PageReader 已有14类分发、练习保存恢复、页内笔记、翻页/书签/已读进度。interactive/animation/concept_graph/figure等仅显式模拟形态，须逐类核参考交互；渲染类型数量不等于完整验收。**H1-BOOKS-PIPELINE v2（2026-09-20）**：页/块六态（排队·规划·编译中·就绪·部分失败·失败）+ 生成中/排队占位分两态 + 块失败卡与单块重试 + 页失败面板与整页重生成（保留笔记与块身份）+ 未生成页打开不登记已读 + 作答版本关系（`contentVersion` 由执行器按内容写入）；三视口/焦点/reduce/中断/快速开关实测见 [批次证据](../qa/H1-BOOKS-PIPELINE/README.md)。**B-R05-EXTEND v2 视觉推广未含此页**（样式文件共享 `books.css`，当时未触碰 PageReader） | **H1-BOOKS-HARDEN v1（2026-09-22）**：页/块修复返回真实异步结果（失败/被取代/不可写都如实提示），启动冻结 runId 且旧任务不得借用新 runId 写入，重复点击互斥；笔记输入框与 contenteditable 内的 ←/→ 不再触发全局翻页（普通阅读仍可翻页）。
| P-courses | `/courses` | `/courses` | 实现待验收 | 课程目录（S5-C）：进行中/已归档折叠区、演示载入（幂等）、新建（颜色标记）；主导航入口按参考隐藏（hidden ready，路由可达）；**资源目录故障容错已修（R-11，2026-09-14）**：知识/笔记本/书籍目录读取失败时课程正文与大纲保留、显示错误与重试、不冒充空、不丢引用；e2e `course-resource-faults.spec.ts`。**B-R05-EXTEND v2（2026-09-18）视觉推广**：卡片悬浮语言、页脚资料数图标化 + 空资料直述（不显示未接入的会话数）、归档折叠头视觉过渡、创建按钮 busy；入口与归档结构按队长裁定保留；**UX-REGRESSION-FIX v1（2026-09-23）**：列表页头部新增「返回教材资料库」固定入口；详情页既有「返回课程列表」保留；e2e 覆盖「教材资料库 → 课程列表 → 课程详情 → 课程列表 → 教材资料库」与手机视口。 |
| P-courses-[courseId] | `/courses/[courseId]` | `/courses/[courseId]` | 部分实现 | 已有大纲逐行编辑、覆盖标记、下一单元提示、资料附加/移除、失效资源提示、约定与归档恢复。**H1-COURSE-SESSIONS v1（2026-09-22）**：新增本课程学习会话区——只按稳定 `courseId` 过滤（不按标题/最近访问猜测）、加载/空态/读取失败重试、新建会话保存成功后才跳转（同一 tick 连点只建一条、写入失败保留页面并给「重试新建」）、归档课程只读；页内说明如实标注（资源仅登记未解析/未检索、RAG 未接入、covered 为学员手判）。e2e `course-sessions.spec.ts` + `books-courses.spec.ts`（原有切片保留）。**B-R05-EXTEND v2（2026-09-18）**：大纲进度条（progressbar + 300ms 宽度过渡）、单元编号/covered 删除线/下一单元高亮/topics 截断；资料行 kind 图标 + 不可用后缀独立 shrink-0 + 移除钮 hover/focus 显隐；保存与归档 busy；R-11 三态经 A1 独立回归（含重试链路与损坏数据逐字节未变）。聚合磁贴/Mode-Persona 字段按裁定属 H1 |
...
| 状态 | 页面范围 | 当前证据与缺口 |
| --- | --- | --- |
| 基准页 | `P-chat` | 蓝色主题、字体 token（`--font-ui` 正文 / `--font-display` 标题 / `--font-ui-serif` 导航与聊天）、220/56px 导航（含可滚动学习记录）、912px 对话列、模型弹层和思考球是全站来源 |
...
| 部分验收 | `P-courses-[courseId]`（学习会话区） | **H1-COURSE-SESSIONS v1（2026-09-22）**：课程会话列表/新建/恢复/课程↔聊天往返 + 轮次课程快照进入真实请求链路的**行为**证据（e2e + 单测 + 后端 system 转发用例）。**不升级**该页的视觉标签（本批未做新的视觉验收）；真实供应商调用 not_run |
...
| 部分验收 | `P-lesson-plans` | **B-R05-EXTEND v4（2026-09-19）**：表单分区/type-chip/模板卡、预览工具栏 hover/disabled、toast/storage-alert 视觉、导出菜单进场 180ms。A1 **pass**（`verifyDocx` 导出全项、损坏草稿不覆盖、草稿恢复、跨页返回）；导出与草稿链路零改动见 [报告](../qa/B-R05-EXT4/A1-REPORT.md)。教案密集编辑区不强制 Lora（`--serif` 回退链保留）；硬编码色未替换、预览缩放过渡未加（按裁定保留）。**UX-REGRESSION-FIX v1（2026-09-23）**：顶栏标题与导出菜单同行（四视口几何实测：标题在顶栏内、与导出同行、不重叠、无溢出、内容区紧接顶栏） |
