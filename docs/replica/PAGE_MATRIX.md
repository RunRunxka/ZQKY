...
更新：2026-09-23（UX-PERF-CLOSEOUT v1 更新 P-chat 与教案行；不升级条目验收状态）。全站视觉以当前学习问答为准；模块信息结构/功能/动画对照固定 DeepTutor。下表“状态”是**功能状态**；页面内容是否按学习问答完成视觉验收在文末单列，二者不得互相替代。根级 NavigationPreference 的公共壳证据不代替各页面内容证据。
...
| P-chat | `/chat` | `/chat` | 部分实现 | 当前全站视觉基准：220/56px 导航（导航内可滚动「学习记录」区域）、912px 内容上限、输入区图标/字体/思考球与模型弹层。普通真实聊天及本地历史已有；运行时模拟服务与切换入口已删除，R-03生产mock残留已清理，复杂能力真实通道未接。历史主页和 H0 侧栏回归不代表全产品或真实供应商通过。**CHAT-CONTEXT-BUDGET v1（2026-09-23）**：请求发送前统一预算（课程块+历史+当前问题）与课程字段限幅，超限时如实提示并保留输入；账目 `requestBudget` 随消息持久化（字符估算）。**UX-PERF-CLOSEOUT v1（2026-09-23）**：学习记录并入全站导航（删除 236px 独立中栏与其折叠按钮/手机第二套弹窗）；模式菜单单层化并以「RAG 模式」取代「更多能力」（未接入、不可选、行内标注）；长推理流卡顿收口（50k 字推理帧间隔 p95 166.7→5.6 ms、0 帧 > 50 ms、主线程脚本 28.3→1.1 s，原始文本逐字节保留）；`/chat` 界面文字改用界面衬线 token。**功能/视觉标签不变**（真实供应商与真实 RAG 仍 not_run）。 |
