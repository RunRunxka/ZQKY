...
2026-09-08 复核补充：本次全量 e2e 的 chat-motion 再次在 context teardown 超过45秒，53通过/1失败。关键帧及trace已产生；本轮查看了过程展开中间帧，未播放整段WebM。根因未定，不能以旧轮“未复现”销项；详细证据见 [最新复核](REVIEW_S2_S3_2026-09-08.md)。
...
| M-sidebar-panel | 侧栏/右面板宽度 | 逐组件核对原版 | 待实现 | S2/S3 |
...
| M-artifact | 产物面板展开 | 逐组件核对原版 | 待实现 | S3 |
| M-buttons | 按钮悬停/按下/禁用/加载 | 逐组件核对原版 | 待实现 | S7 |
| M-ask-states | 追问卡状态切换（预览→可答→提交→摘要） | AskUserOptions 实际过渡 | 待实现 | 原版无装饰进场；状态切换动画待逐项核对（S7） |

动画证据边界（2026-09-07）：扩展菜单/工具面板有整段录像+定格帧并人工查看；追问卡以 animationName 断言记录"无装饰动画"，其状态切换的动态验收待 S7。chat-motion 录像用例 teardown 超时：复核轮 1 次、本轮 3 连跑未复现，根因未知（见 HANDOFF）。
