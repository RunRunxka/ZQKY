...
- **学习问答**：三协议真实 SSE 流式、推理/正文与公式渲染、来源消息定位、本地会话与轮次快照
...
- **公共壳**：220/56px 侧栏、折叠偏好、手机模态抽屉、统一 404 与主题

## 🛠️ 技术栈

- **框架**：Next.js 16（App Router）+ React 19 + TypeScript 5.7
- **样式**：Tailwind CSS 3.4 + 单一变量层（蓝色主题、Chat Geist/Lora 字体）
- **状态**：Zustand 5
...
`/chat` 是全站默认主页与视觉基准：真实 FastAPI 三协议 SSE、推理与正文分栏、公式、来源消息定位、会话与草稿本地持久化。生产模拟服务与入口已清理，复杂能力（工具、附件解析等）未接入前明确不可用。
...
知识库为显式模拟的登记→解析→索引流程（进度/取消/重试/恢复），真实文件解析与向量检索未接入；书籍/课程提供 block 渲染、章节大纲、练习、笔记、资源与进度。各模块未完成状态以 [STATUS](docs/STATUS.md) 台账为准。
